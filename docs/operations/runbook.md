...
| Query results are slow on cold semantic misses | `free-ai` gateway embedding (`gemini-embedding-001`) + Vectorize request paid cold-path latency | Prefer lexical/auto for exact-term UX, cache popular queries, or use precomputed query vectors where possible. |
