...
| Dense vector search | Vectorize | Done; implemented in Worker with corrective lexical fallback, in-Worker BM25 fuzzy sparse lexical scoring over D1 chunks, and optional Workers AI neural rerank |
...
| Full hybrid retrieval and query answers | Vectorize plus in-Worker BM25 sparse lexical scoring over D1 chunks, app-side fusion, and free-ai/Workers AI synthesis | Done; explicit `hybrid` mode fuses in-Worker BM25 fuzzy sparse lexical and Vectorize results with RRF, lexical paths apply deterministic rewrite/decompose fanout, explicit `semantic` mode corrects weak/empty Vectorize evidence with lexical RRF fallback, applies Worker-native keyword rerank/MMR plus opt-in Workers AI neural rerank, structured queries support exact field filters plus D1 relationship graph evidence, `/v1/kb/query` has extractive answers by default plus opt-in Workers AI cited synthesis, and `/v1/kb/query/stream` preserves the old SSE lifecycle stream contract |
...
Cloudflare-native retrieval stack: Vectorize dense search; an in-Worker BM25
sparse lexical scorer over D1 chunks (`sparseLexicalScore`, `k1 = 1.2`,
`b = 0.75`, IDF + length-normalized TF, fuzzy token matching) that loads the
index's chunks via `listChunksForIndex` (capped at 5000, cached per
tenant/index) and scores them in the isolate; app-side RRF fusion; local
rerank/MMR; D1 graph expansion; and optional Workers AI neural rerank. This is
functional retrieval parity, not the exact BM42 model.
...
- `/v1/kb/search`, `/v1/indexes/:id/query`, and `/v1/kb/query` support
  explicit `hybrid` mode, which fuses in-Worker BM25 sparse lexical scoring over
  D1 chunks and Vectorize
