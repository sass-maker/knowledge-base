...
function failRegressions(label, observed, baseline) {
	...
	if (
		...
		console.log(
			`${label} improved; lower the checked-in baseline in the next intentional update.`,
		);
	}
}
...
function checkComplexity() {
	...
	const observed = {
		...
		violations: rows.filter((row) => row[1] > 20 || row[4] > 100 || row[3] > 7)
			.length,
...
function audit(directory) {
	const report = JSON.parse(
		run("pnpm", ["--dir", directory, "audit", "--json"], { allowFailure: true })
			.stdout,
	);
...
function checkDependencies() {
	...
	for (const [scope, report] of Object.entries(reports)) {
		...
		if (unexpected.length > 0) {
			throw new Error(
				`Unaccepted ${scope} critical/high advisories: ${unexpected.join(", ")}`,
			);
		}
	}
	const observed = {
