import { createAppHealthClient, type AppHealthClient, type AppHealthClientOptions } from '@saas-maker/app-health';
...
type AppHealthBindings = Pick<Env, 'APP_HEALTH_ENVIRONMENT' | 'APP_HEALTH_INGEST_KEY' | 'RAG_DEPLOY_FINGERPRINT'>;

/** Create one bounded client per request, or stay inert until a private key exists. */
export function createKnowledgeBaseAppHealthClient(env: AppHealthBindings, fetchOverride?: AppHealthClientOptions['fetch']): AppHealthClient | null {
...
export function knowledgeBaseAppHealthMiddleware(makeClient: AppHealthClientFactory = createKnowledgeBaseAppHealthClient) {
