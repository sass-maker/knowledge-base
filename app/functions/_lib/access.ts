...
export async function verifyOperator(request: Request, env: AccessEnv): Promise<OperatorIdentity> {
...
export function accessErrorResponse(error: unknown): Response {
  ...
  return Response.json({ error: 'Operator authentication failed' }, { status: 401 });
}
