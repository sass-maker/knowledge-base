...
import { LayoutDashboard, Database, Search, FileUp, FlaskConical, History, Settings, TableProperties, ScrollText } from 'lucide-react';
...
export function Sidebar() {
  ...
  const { projects, selectedProject, includeInternal, setSelectedProject, setIncludeInternal } = useProjectScope();
  return (
          ...
          >
            {projects.map((project) => (
              <option key={project.name} value={project.name}>
                {projectLabel(project.name)} · {project.file_count} {project.file_count === 1 ? 'file' : 'files'}
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? 'nav-item-active bg-primary/10 text-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground hover:translate-x-0.5',
              )}
            >
