...
import { api, type Domain, type SearchResults, type QueryResult, ApiError } from '@/lib/api';
...
export default function QueryPage() {
  ...
  return (
    <>
      <PageHeader title="Query" description="Search your corpus or get cited answers" />
          ...
          <Card>
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-base font-semibold text-foreground">No domains to query</h3>
                <p className="max-w-sm text-sm text-muted-foreground">Create a domain and ingest some files first, then come back to run queries.</p>
              </div>
              <Link
                href="/domains"
          ...
          <>
            ...
            <Card>
              <div className="flex flex-col gap-4">
                ...
                <label className="flex flex-col gap-1.5">
                  <span className="text-xs font-medium text-muted-foreground">{tab === 'search' ? 'Query' : 'Question'}</span>
                  <textarea
                    value={query}
                ...
                <div className="flex items-center gap-2">
                  <div className="flex rounded-lg border border-border p-0.5">
                    ...
                    <button
                      ...
                      className={cn(
                        'flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-colors',
                        tab === 'answer' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground',
                      )}
                    >
            ...
            {tab === 'search' && searchResults && (
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>
                    {searchResults.results.length} results · {searchResults.mode}
                  </span>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs text-muted-foreground">{r.document}</span>
                        </div>
                        <p className="text-sm leading-relaxed text-foreground">{r.content}</p>
                      </div>
                      <span className="shrink-0 font-mono text-xs text-muted-foreground">{formatScore(r.score)}</span>
                    </div>
                  </Card>
                ))}
              </div>
            ...
            {tab === 'answer' && queryResult && (
              <div className="flex flex-col gap-4">
                <Card>
                  ...
                  <p className="text-sm leading-relaxed text-foreground">{queryResult.answer}</p>
                  <div className="mt-4 flex flex-wrap gap-4 text-xs text-muted-foreground">
                    <span>Mode: {queryResult.mode}</span>
                    <span>Latency: {formatMs(queryResult.latency_ms)}</span>
                    {queryResult.confidence !== null && <span>Confidence: {formatScore(queryResult.confidence)}</span>}
                    {queryResult.trace_id && (
                      <Link href={`/history?trace=${encodeURIComponent(queryResult.trace_id)}`} className="text-accent underline-offset-4 hover:underline">
                ...
                {queryResult.citations.length > 0 && (
                  <div className="flex flex-col gap-3">
                    <h3 className="text-sm font-semibold text-foreground">Sources ({queryResult.citations.length})</h3>
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-mono text-xs text-muted-foreground">{c.document}</span>
                            </div>
                            <p className="text-sm leading-relaxed text-foreground">{c.content}</p>
                          </div>
                          <span className="shrink-0 font-mono text-xs text-muted-foreground">{formatScore(c.score)}</span>
                        </div>
                      </Card>
                    ))}
                  </div>
