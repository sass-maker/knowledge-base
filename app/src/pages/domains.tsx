...
import { Briefcase, Database, FileText, GitBranch, Loader2, Play, Plus, RefreshCw, Upload } from 'lucide-react';
...
export default function DomainsPage() {
  ...
  const loadDomains = useCallback(async () => {
    try {
      const [d, m] = await Promise.all([api.getDomains(), api.getEmbeddingModels()]);
            ...
            <div className="flex flex-col gap-3">
              ...
              {models.length > 0 && (
                <label className="flex flex-col gap-1.5">
                  ...
                  <span className="text-xs text-muted-foreground">Can&apos;t be changed after ingestion.</span>
                </label>
              )}
          ...
          <Card>
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-base font-semibold text-foreground">No domains yet</h3>
                <p className="max-w-sm text-sm text-muted-foreground">
                  Create your first domain to start ingesting files, inferring schemas, and running cited queries.
                </p>
              </div>
              ...
              {domains.map((d) => (
                <button
                  ...
                  className={`card-lift flex flex-col gap-1 rounded-lg border px-4 py-3 text-left ${
                    selected === d.name ? 'border-accent/40 bg-accent/5' : 'border-border bg-card'
                  }`}
                >
              ...
              <div className="flex flex-col gap-6">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="flex flex-col gap-1 text-sm">
                        <span className="font-mono text-foreground">{activeSchema ? `${activeSchema.name} v${activeSchema.version}` : 'No active schema'}</span>
                        <span className="text-xs text-muted-foreground">
                    ...
                    <Button
                      ...
                      onClick={() => runAction('Queued schema reprocess', () => api.reprocessDomainSchema(selected))}
                    >
                        ...
                        >
                          ...
                          <div className="flex shrink-0 gap-2">
                            <Button
                              ...
                              variant="secondary"
                              disabled={busy !== null}
                              onClick={() => runAction('Applied schema draft', () => api.applySchemaDraft(draft.id))}
                            >
                              Apply
                            </Button>
                            <Button
                              ...
                              variant="ghost"
                              disabled={busy !== null}
                              onClick={() => runAction('Discarded schema draft', () => api.discardSchemaDraft(draft.id))}
                            >
                              Discard
                            </Button>
                          </div>
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                  <Card>
                    ...
                    {files.length === 0 ? (
                      <div className="flex flex-col gap-3">
                        <p className="text-sm text-muted-foreground">No files in this domain yet.</p>
                        <Link
                          href="/ingest"
                      ...
                      <div className="flex flex-col gap-2">
                        {files.slice(0, 12).map((f) => (
                          ...
                          >
                            <div className="min-w-0">
                              <div className="truncate font-mono text-xs text-foreground">{f.filename}</div>
                              <div className="text-xs text-muted-foreground">
                            ...
                            <Button
                              ...
                              variant="ghost"
                              disabled={busy !== null}
                              onClick={() => runAction('Queued file reprocess', () => api.reprocessFile(f.id))}
                            >
                              <Play className="size-3.5" /> Replay
                            </Button>
                          </div>
                        ))}
                        {files.length > 12 && <span className="text-xs text-muted-foreground">+{files.length - 12} more files</span>}
                      </div>
                    )}
                  </Card>
                  ...
                  <Card>
                    ...
                    {jobs.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No jobs for this domain yet.</p>
                    ) : (
                      <div className="flex flex-col gap-2">
                        {jobs.slice(0, 12).map((j) => (
                          <div key={j.id} className="rounded-lg border border-border bg-background/50 px-3 py-2 text-sm">
                            <div className="flex items-center justify-between gap-3">
                              <span className="flex items-center gap-2">
                                <span
                                  className={`size-2 rounded-full ${
                                    j.status === 'done' ? 'bg-emerald-500' : j.status === 'failed' ? 'bg-red-500' : 'bg-yellow-500'
                                  }`}
                                />
                                <span className="font-mono text-xs text-foreground">{j.stage}</span>
                              </span>
                              <span className="text-xs text-muted-foreground">
                                {j.status} · {formatTime(j.created_at)}
                              </span>
                            </div>
                            {j.error && <p className="mt-1 line-clamp-2 text-xs text-destructive">{j.error}</p>}
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                  <Card>
                    <div className="mb-3 flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs text-muted-foreground">{entities.length} shown</span>
                    </div>
                    {entities.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No extracted entities for this domain yet.</p>
                    ) : (
                      <div className="flex flex-col gap-2">
                        {entities.slice(0, 10).map((entity) => (
                          <div key={entity.id} className="rounded-lg border border-border bg-background/50 px-3 py-2 text-sm">
                            <div className="truncate text-foreground">{entity.display_name ?? entity.identity_key}</div>
                  ...
                  <Card>
                    <div className="mb-3 flex items-center justify-between gap-3">
                      ...
                      <Button
                        ...
                        variant="secondary"
                        disabled={!activeSchema || busy !== null}
                        onClick={() => runAction('Backfilled relationships', () => api.backfillRelationships(selected))}
                      >
                        Backfill
                      </Button>
                    </div>
                    {relationships.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No relationships for this domain yet.</p>
                    ) : (
                      <div className="flex flex-col gap-2">
                        {relationships.slice(0, 10).map((rel) => (
                          <div key={rel.id} className="rounded-lg border border-border bg-background/50 px-3 py-2 text-sm">
                            <div className="truncate text-foreground">
                              {rel.source_display_name ?? rel.src_id} → {rel.target_display_name ?? rel.dst_id}
                            </div>
                            <div className="font-mono text-xs text-muted-foreground">{rel.rel_type}</div>
                          </div>
                        ))}
                      </div>
                    )}
