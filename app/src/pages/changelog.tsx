      ...
      <main className="mx-auto flex w-full max-w-4xl flex-col gap-6 p-6">
        <div className="max-w-2xl">
          <p className="text-sm leading-6 text-muted-foreground">
            This is the maintained product history. Open and planned work stays in GitHub Issues, while each entry here reflects verified repository or
            deployment evidence.
          </p>
          <div className="mt-4 flex flex-wrap gap-2 md:hidden">
            <a href={ROADMAP_URL} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium">
              Roadmap <ExternalLink className="size-3.5" />
            </a>
            <a href={SOURCE_URL} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium">
        ...
        <section className="flex flex-col" aria-label="Knowledgebase releases">
          {entries.map((entry) => (
            <article key={`${entry.date}-${entry.title}`} className="grid gap-3 border-t border-border py-6 sm:grid-cols-[140px_minmax(0,1fr)] sm:gap-6">
              <time dateTime={entry.date} className="font-mono text-xs font-medium text-muted-foreground">
                {entry.label}
              </time>
              <Card className="p-5">
                <h2 className="text-base font-semibold text-foreground">{entry.title}</h2>
                <p className="mt-2 max-w-[70ch] text-sm leading-6 text-muted-foreground">{entry.body}</p>
                <ul className="mt-4 flex flex-wrap gap-2" aria-label="Update categories">
                  {entry.tags.map((tag) => (
                    <li key={tag} className="rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-foreground">
                      {tag}
                    </li>
                  ))}
                </ul>
